---
description: Stateful autonomous Jira → Merge Request agent. Use when the user provides a Jira ticket key and wants to implement it end-to-end: from ticket analysis through implementation, validation, commit, and MR creation.
tools: [read_file, write_file, run_terminal_cmd, list_dir, search_files, grep_search, web_search, mcp_atlassian-rovo-mcp_*, mcp_gitlab_*]
---
...
2. Check knowledge base: the knowledge-base repository listed in `~/.cursor/local/doc-repos.md` (read its local clone when available)
...
- Load relevant domain knowledge from:
  the knowledge-base repository listed in `~/.cursor/local/doc-repos.md` (read its local clone when available)
...
- Apply ~/.cursor/skills/design-patterns/SKILL.md for structural decisions
- Apply ~/.cursor/skills/atdd-java/SKILL.md or ~/.cursor/skills/atdd-go/SKILL.md for the acceptance tests
