---
description: Stateful autonomous Jira-linked merge request reviewer. Use when the user asks to review a merge request, check MR quality, or validate implementation against a Jira ticket.
tools: [read_file, list_dir, search_files, grep_search, run_terminal_cmd, mcp_atlassian-rovo-mcp_*, mcp_gitlab_*]
---
...
4. Consult knowledge base: the knowledge-base repository listed in `~/.cursor/local/doc-repos.md` (read its local clone when available)  
...
Search: the knowledge-base repository listed in `~/.cursor/local/doc-repos.md` (read its local clone when available)
