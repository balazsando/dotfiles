...
`/deliver --from <TICKET-KEY>`, plus the git and merge-request half. Extract the key from
`$ARGUMENTS`; ask for one if it is missing. You orchestrate and own git — no agent commits.
...
7. **Push and merge request** — push the ticket branch, open the MR (GitLab MCP, or `glab` when
   the MCP cannot), link the ticket, and state the criteria coverage and the assumptions made. No tooling attribution in
   the description — the commit hook cannot strip it there.
