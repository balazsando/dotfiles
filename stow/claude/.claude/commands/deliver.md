...
Deliver `$ARGUMENTS` as a team of narrow agents. You are the orchestrator: you route, you do not
design, implement, test, or document. Prepares files only — no branch, no commit.
...
Spawn each agent with the Agent tool, in the foreground, one at a time — the deep parallel pair
below is the only exception. Give it the paths and the one-line task — never the previous
agent's output pasted in, never the original prompt when a brief has replaced it.
...
2. **Spawn both** in one tool block, each with its own paths and working directory.
...
- `BLOCKED: <business question>` → ask the **user**, then resume the same agent via SendMessage.
