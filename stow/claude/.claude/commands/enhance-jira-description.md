...
argument-hint: "<TICKET-KEY> [instructions]"
---

Rewrite the description of the ticket in `$ARGUMENTS`. Empty → ask for the key. Any further text
...
3. **The documentation repositories** named in `~/.claude/local/doc-repos.md`, for the "why" —
   only when 1–2 do not supply it. Missing file → say so and skip this source.
