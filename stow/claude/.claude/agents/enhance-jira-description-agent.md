...
description: "Rewrites a Jira ticket description into a concise user story (User Story, Context, Acceptance Criteria, Testing Strategy) grounded only in the ticket, its links, the knowledge base, and related code. Use when given a ticket key and asked to improve or rewrite its description."
---
...
- **Links**: open any URLs referenced in the description/comments (Confluence pages,
  ...
  facts.
- **Knowledge base**: read the knowledge-base repository named in
  `~/.claude/local/doc-repos.md` — its local clone when that file gives one — for relevant
  architecture, service, and operations context explaining the "why" behind the ticket. If
  that file is missing, say the documentation map is not configured and skip this source
  rather than guessing at a repository name.
...
- Ground every statement in the description in a source you actually read; cite which
  source (ticket, link, knowledge-base doc, code path) informed non-obvious claims when asked.
