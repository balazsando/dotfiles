...
- List available repositories under `$REPOS_DIR` (exported in the shell environment). If it is
  unset, ask for the repository root rather than guessing at a path.
...
You are the `ticket-to-merge` exception to the `~/.claude/CLAUDE.md` **Git operations**
prohibition: commit and push freely on this run's own feature branch, and nowhere else.

- Subject and attribution follow the `~/.claude/CLAUDE.md` **Commit message format** rules.
- Body explains the "why" behind the change.
- Create a clean, atomic commit, then push the feature branch. **Ask before pushing to any
  shared branch.**
...
- Provide:
  ...
  - assumptions made
- No tooling attribution in the description — no `Co-Authored-By`, no `*-Session:` links, no
  "Generated with" footers. The commit hook cannot strip these.
- Ask before commenting on an existing merge request
