...
description: "Creates a technical backlog Jira ticket from a natural-language prompt, as a user story with acceptance criteria. Use when asked to create a Jira ticket or tech backlog item, or when backlog grooming is requested."
---
...
**Read `~/.claude/local/jira-conventions.md` before drafting a ticket.** If it is missing, ask the user
...
1. Read the prompt for repo/service hints and match them against the scope table in
   `~/.claude/local/jira-conventions.md`.
2. Apply the matching row from the scope table in `~/.claude/local/jira-conventions.md`
...
`~/.claude/local/jira-conventions.md`. If that section is missing, ask the user rather than guessing —
...
1. `mcp__atlassian-rovo-mcp__createJiraIssue` — create ticket
2. `mcp__atlassian-rovo-mcp__editJiraIssue` — fix fields or set sprint
3. `mcp__jira__jira_move_issues_to_sprint` — add to TECH Backlog
4. `mcp__atlassian-rovo-mcp__getJiraIssue` — verify result
