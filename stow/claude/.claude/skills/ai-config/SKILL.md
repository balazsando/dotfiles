...
| Layer | Owns | Loaded |
| --- | --- | --- |
| **Router** — `CLAUDE.md` (Claude only) | Situation → skill, plus the standing contracts (git, documentation, communication, machine-local) | Every Claude request |
...
- `rules/*.mdc` — one concern per file. Always-on rules mirror a `CLAUDE.md` section: `git`,
  `documentation`, `layers` (precedence, machine-local), `economy-of-words`. `globs` rules:
  `java`, `neovim`.
