...
`$ARGUMENTS` sets the scope: paths narrow it to those files, a rev range (`<tag>..HEAD`) audits
that range plus the working tree, empty audits the staged set.

Reports only. Nothing is written or fixed, and staging, unstaging and committing stay with the
user — the **Git operations** rules in `~/.claude/CLAUDE.md` are not suspended for this command.
...
3. **Read for findings**:
   ...
   - **Inconsistency** — two tracked sources contradicting each other: `README.md` against the
